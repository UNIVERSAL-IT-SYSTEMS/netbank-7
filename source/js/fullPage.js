...
function FullPage(options) {
	...
	if (_isNav) {
		navChange = function(from, to) {
			document.body.className = 'bodypage'+(to-0+1);
			for(var i = 0; i< navObj.length; i++){
				var navChildren = navObj[i].children;
			...
			switch (true) {
				...
				(function() {
					var navId = m.split(':')[1],						
						...
						_t;
					navObj = $("."+navId);
					for(var cd = 0; cd<navObj.length; cd++){
						var _navObj = navObj[cd];

					var navChildren = _navObj.children;
					navLen = navChildren.length;
					_t = navChildren[indexNow].className;

					if (!_navObj || !navChildren) return;
					...
					if(document.body.className.indexOf("page")===-1){
						document.body.className = "bodypage1";
					}
					gotoPage = function(e) {
